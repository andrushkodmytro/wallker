import React, { Component } from 'react';
import '../Settings.css';
import '../../../../../assets/fonts1/fonts.css';

export default class SettingsPassword extends Component {
    render() {
      return (
        <div className="container_settings__password">
            <div className="container_settings__password-block">
              <div className="profile_text">
                  <p>Password</p>
                </div>
                <div className="inputs">
                  <div class="current_password">
                    <label class="password__label">Current&nbsp;Password</label>
                    <input class="password__input" type="password"/>
                  </div>
                  <div class="new_password">
                    <label class="password__label">New&nbsp;Password</label>
                    <input class="password__input" type="password"/>
                  </div>
                  <div class="confirm_password">
                  <label class="password__label">Confirm&nbsp;New&nbsp;Password</label>
                  <input class="password__input" type="password"/>
                </div>
                </div>
                <div className="change_block">
              <button class="change_block__button">Change</button>
            </div>
            </div>
        </div>
      );
    }
  }